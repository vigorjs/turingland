...
export default function AdminDeveloperPage({ developers, auth }) {
    ...
    const sortedData = [...developers.data].sort((a, b) => {
        if (sortOrder === "asc") {
            return a.id - b.id;
        } else {
            return b.id - a.id;
    ...
    const handleOpenEditModal = (developer) => {
        setDeveloper(developer);
        setIsOpenModal(true);
    };

    const handleOpenDeleteModal = (developer) => {
        setDeveloper(developer);
    ...
    const handleCheckboxChange = (e, id) => {
        if (e.target.checked) {
            ...
            setSelectedDevelopers((prev) =>
                prev.filter((developerId) => developerId !== id)
            );
        }
    };
                            ...
                            <table className="w-full rounded-xl">
                                ...
                                <tbody className="divide-y divide-gray-300">
                                    {sortedData
                                        ...
                                        .map((developer, index) => (
                                            <tr
                                                key={`developer-${index}-${developer.id}`}
