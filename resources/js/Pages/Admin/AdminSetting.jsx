...
const AdminSettings = () => {
    ...
    useEffect(() => {
        const fetchImage = async (key, setter) => {
            try {
                const response = await axios.get(route('web-preferences.get', { key }));
    ...
    const handleFileSubmit = (key, file) => async (e) => {
        e.preventDefault();
        if (!file) {
            alert(`Pilih file untuk ${key.replace("_", " ")} terlebih dahulu.`);
            return;
        }
        ...
        try {
            ...
            console.error(error);
            alert(`Gagal mengunggah ${key.replace("_", " ")}.`);
        }
    };
    ...
    const handleLogoDarkChange = handleFileChange(setLogoDark, setPreviewLogoDark);
    const handleImgLoginChange = handleFileChange(setImgLogin, setPreviewImgLogin);
    const handleImgRegisterChange = handleFileChange(setImgRegister, setPreviewImgRegister);
    ...
    const handleImgRegisterSubmit = handleFileSubmit("img_register_url", imgRegister);
        ...
        <AdminLayout>
            <div className="p-6 bg-white rounded-lg shadow-md">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <UpdateImageForm
                        title="Logo"
                        currentImage={currentLogo ? `/storage/${currentLogo}` : null}
                    ...
                    <UpdateImageForm
                        title="Logo Dark"
                        currentImage={currentLogoDark ? `/storage/${currentLogoDark}` : null}
                    ...
                    <UpdateImageForm
                        title="Hero"
                        currentImage={currentHero ? `/storage/${currentHero}` : null}
                    ...
                    <UpdateImageForm
                        title="Login Image"
                        currentImage={currentImgLogin ? `/storage/${currentImgLogin}` : null}
                    ...
                    <UpdateImageForm
                        ...
                        currentImage={currentImgRegister ? `/storage/${currentImgRegister}` : null}
