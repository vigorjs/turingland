...
import ModalTestimonyForm from "../../../Components/admin/testimony/ModalTestimonyForm";
...
export default function AdminTestimonyPage({ testimonies, auth }) {
    ...
    const sortedData = [...testimonies.data].sort((a, b) => {
        if (sortOrder === "asc") {
            return a.id - b.id;
        } else {
            return b.id - a.id;
    ...
    const handleOpenEditModal = (testimony) => {
        setTestimony(testimony);
        setIsOpenModal(true);
    };

    const handleOpenDeleteModal = (testimony) => {
        setTestimony(testimony);
        setIsOpenDeleteModal(true);
    };

    const handleDeleteTestimony = () => {
        router.delete(route("testimony.destroy", testimony.id), {
            onSuccess: () => {
                toast({
    ...
    const handleCheckboxChange = (e, id) => {
        if (e.target.checked) {
            setSelectedTestimonies((prev) => [...prev, id]);
        } else {
            setSelectedTestimonies((prev) =>
                prev.filter((testimonyId) => testimonyId !== id)
            );
        }
    };
    ...
    return (
        <AdminLayout auth={auth} title="Testimony">
            ...
            <div className="flex flex-col">
                <div className=" overflow-x-auto pb-4">
                    <div className="block">
                            ...
                            <table className="w-full rounded-xl">
                                <thead>
                                    <tr className="bg-gray-50">
                                        <th className="px-5 py-2.5 text-left whitespace-nowrap text-sm leading-6">
                                            <Checkbox
                                                type="checkbox"
                                                checked={selectAll}
                                ...
                                <tbody className="divide-y divide-gray-300">
                                    {sortedData
                                        .filter((testimony) => {
                                            return (
                                                testimony.client_name
                                                    .toLowerCase()
                                                    .includes(
                                                        filter.client_name.toLowerCase()
                                                    ) &&
                                                testimony.content
                                                    .toLowerCase()
                                                    .includes(
                                        ...
                                        .map((testimony, index) => (
                                            <tr
                                                key={`testimony-${index}-${testimony.id}`}
                                                className="bg-white transition-all duration-500 hover:bg-gray-200"
                                            >
                                                <td className="px-5 py-2.5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    <Checkbox
                                                        type="checkbox"
                                                        checked={selectedTestimonies.includes(
                                                            testimony.id
                                                        )}
                                                        onChange={(e) =>
                                                ...
                                                <td className="px-6 py-4 w-24">
                                                    <img
                                                        src={
                                                            testimony.client_avatar
                                                                ? `/storage/${testimony.client_avatar}`
                                                                : "/assets/avatar.png"
                                                        }
                                                        alt={
                                                            testimony.client_name
                        ...
                        {/* PAGINATION */}
                        <Pagination
                            prev_page_url={testimonies.prev_page_url}
                            first_page_url={testimonies.first_page_url}
                            last_page_url={testimonies.last_page_url}
                            links={testimonies.links}
                            next_page_url={testimonies.next_page_url}
                        />
                    </div>
                </div>
            </div>
            ...
            <AlertConfirmModal
                ...
                title="Hapus Testimony"
                message="Apakah anda yakin ingin menghapus testimony ini?"
