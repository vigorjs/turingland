...
import AgentStatusBadge from "@/Components/user/AgentStatusBadge";
import ModalUserForm from "@/Components/user/ModalUserForm";
...
export default function AdminAgentPage({ agents, auth }) {
    ...
    const sortedData = [...agents.data].sort((a, b) => {
        if (sortOrder === "asc") {
            return a.id - b.id;
        } else {
            return b.id - a.id;
    ...
    const filteredData = sortedData.filter((agent) => {
        ...
        const phoneMatch = agent.wa_number
            .toLowerCase()
        ...
        const statusMatch =
            filter.status === "" ||
            (filter.status === "1" && agent.is_agent_active === true) ||
            (filter.status === "0" && agent.is_agent_active === false);
    ...
    const handleSelectAll = (e) => {
        if (e.target.checked) {
            setSelectAll(true);
            setSelectedAgents(agents.data.map((agent) => agent.id));
        } else {
            setSelectAll(false);
            setSelectedAgents([]);
        }
            ...
            <div className="flex flex-col">
                <div className=" overflow-x-auto pb-4">
                    <div className="block">
                            ...
                            <table className="w-full rounded-xl">
                                <thead>
                                    <tr className="bg-gray-50">
                                        ...
                                        <th
                                            className="px-2.5 py-2.5 flex items-center gap-1 text-left whitespace-nowrap text-sm leading-6 font-semibold capitalize cursor-pointer transition-all duration-300 ease-in-out hover:bg-gray-200 hover:scale-105 hover:text-primary"
                        ...
                        {/* PAGINATION */}
                        <Pagination
                            prev_page_url={agents.prev_page_url}
                            first_page_url={agents.first_page_url}
                            last_page_url={agents.last_page_url}
                            links={agents.links}
                            next_page_url={agents.next_page_url}
                        />
                    </div>
                </div>
            </div>
