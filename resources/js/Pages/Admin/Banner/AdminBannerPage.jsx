...
import ModalBannerForm from "../../../Components/admin/banner/ModalBannerForm";
...
export default function AdminBannerPage({ banners, auth }) {
    ...
    const sortedData = [...banners.data].sort((a, b) => {
        if (sortOrder === "asc") {
            return a.id - b.id;
        } else {
            return b.id - a.id;
    ...
    const handleDeleteBanner = () => {
        router.delete(route("banner.destroy", banner.id), {
            onSuccess: () => {
                toast({
    ...
    const handleSelectAll = (e) => {
        if (e.target.checked) {
            setSelectAll(true);
            setSelectedBanners(banners.data.map((banner) => banner.id));
        } else {
            setSelectAll(false);
            setSelectedBanners([]);
        }
    ...
    return (
        <AdminLayout auth={auth} title="Banner">
            <Button
                onClick={() => {
                    setIsOpenModal(true);
                    setBanner(null);
                ...
                <div className=" overflow-x-auto pb-4">
                    <div className="block">
                                ...
                                <thead>
                                    <tr className="bg-gray-50">
                                        ...
                                        {[
                                            ...
                                            { key: "image", label: "Image", isStatic: true },
                                            { key: "link", label: "Link" },
                                            { key: "order", label: "Order", isStatic: true },
                                        ...
                                        .filter((banner) => {
                                            return (
                                                ...
                                                banner.link
                                                    .toLowerCase()
                                                    .includes(
                                                        filter.link.toLowerCase()
                                                    ) &&
                                                (filter.status === ""
                                                    ? true
                                                    : banner.is_active.toString() ===
                                                    filter.status)
                                            );
                                        })
                                        .map((banner, index) => (
                        ...
                        <Pagination
                            ...
                            first_page_url={banners.first_page_url}
                            last_page_url={banners.last_page_url}
                            links={banners.links}
