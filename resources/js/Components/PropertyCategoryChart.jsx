...
const PropertyCategoryChart = ({ data }) => {
  ...
  const percentageChange = ((totalProperties - previousTotal) / (previousTotal || 1) * 100).toFixed(1);
      ...
      return (
        <div className="bg-background border border-border rounded-lg p-2 text-sm">
          ...
          <p className="text-muted-foreground">
            {payload[0].value} properties ({((payload[0].value / totalProperties) * 100).toFixed(1)}%)
          </p>
        </div>
      );
    }
  ...
  const renderCustomizedLabel = ({
    ...
    return (
      <g>
        ...
        <path 
          d={`M ${cx + outerRadius * Math.cos(-midAngle * RADIAN)},${cy + outerRadius * Math.sin(-midAngle * RADIAN)} L ${innerPoint.x},${innerPoint.y}`}
  ...
  return (
    <Card className="flex flex-col">
      ...
      <CardContent className="flex-1 pb-0">
        <div className="mx-auto aspect-square h-[300px]"> {/* Increased height for labels */}
